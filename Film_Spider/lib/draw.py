...
def CountDateDB(conn,table_name):
    cursor = conn.cursor()
    sql = "SELECT movie_date,count(*)FROM  "+table_name+"  group  BY movie_date;"
    # print(sql)
    # 执行SQL语句
    cursor.execute(sql)
    results = cursor.fetchall()
...
def CountTimeDB(conn,table_name):
    cursor = conn.cursor()
    sql = "SELECT movie_time,count(*)FROM  "+table_name+"  group  BY movie_time;"
    # print(sql)
    # 执行SQL语句
    cursor.execute(sql)
    results = cursor.fetchall()
